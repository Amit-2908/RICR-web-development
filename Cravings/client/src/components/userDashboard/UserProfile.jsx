...
const UserProfile = () => {
  const [isEditProfileModalsOpen, setIsEditProfileModalsOpen] = useState(false);
  const { user } = useAuth();
  ...
  return (
    <>
      <div className="bg-white rounded-lg shadow-md p-6 md:p-8">
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            ...
            onClick={() => setIsEditProfileModalsOpen(true)}
          >
            Edit Profile
          </button>
        </div>
      ...
      {isEditProfileModalsOpen && (
        <EditProfileModals onClose={() => setIsEditProfileModalsOpen(false)} />
      )}
    </>
  );
};
